// update subjects test case
import { test, expect } from '@playwright/test';
import { initiatePlaywrightRoutes } from 'ftmocks-utils';

test('update subjects', async ({ page }) => {
 await initiatePlaywrightRoutes(
        page,
        {
          MOCK_DIR: '../testMockData',
          FALLBACK_DIR: '../build',
        },
        'update subjects'
      );
  await page.goto('http://localhost:4050/');
  await page.locator("//button[contains(text(), 'Subjects')]").click();
  await page.locator("//*[@id='subject-6-edit-btn']").click();
  await page.locator("input[name='name']").click();
  await page.locator("input[name='name']").click();
  await page.locator("//*[@id='subject-form-name']").dblclick();
  await page.locator("//*[@id='subject-form-name']").type('Sb.Update');
  await page.locator("//button[contains(text(), 'Update')]").click();
  await page.locator("//td[contains(text(), 'Sb.Update')]").click();
  await page.locator("//td[contains(text(), '5')]").click();
  await page.close();
});
